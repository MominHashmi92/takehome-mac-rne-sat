...
### A. Root Cause Analysis — Task 5f436941
...
We edited `docs/spec.md` without changing any requirement (no testbench changes needed), targeting known trouble spots from the hidden test's own FM-1–FM-11 failure-mode tags plus what we confirmed from Task 5f436941:

1. **Clarified the round-half-to-even tie-break checks `q`'s parity, not the rounded result's** — targets FM-1/FM-2 (round-half-up or wrong-sign rounding bugs).
2. **Added an explicit warning that `clr`/`en` form an independent 4-row truth table, not a priority chain** — targets FM-4 (treating clear-then-accumulate as just "clear" or just "accumulate").
3. **Added two worked boundary examples**: a tie that rounds into overflow and must then saturate, and the exact `-32768` case that is *not* an overflow — targets FM-7 (saturate-before-round ordering) and FM-10 (over-eager overflow flagging at the minimum value).

We deliberately excluded a full reference pseudocode algorithm and an explicit `rd`/`en`/`clr` timing table from an earlier draft — that version pushed pass rate to 10/10, effectively reducing the task to transcription. We kept the original prose description of snapshot timing intact, since Task 5f436941's failure was a self-inflicted pipeline error rather than a misunderstanding of the timing rule itself, so over-specifying that section wouldn't address this failure mode and risked flattening the task.
